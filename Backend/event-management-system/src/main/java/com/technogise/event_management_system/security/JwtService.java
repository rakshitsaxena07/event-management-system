...
public class JwtService {
  ...
  private SecretKey
      getSigningKey() { // this method generates a cryptographic key from the secretKey string which
    // is used to sign and verify JWT tokens
    return Keys.hmacShaKeyFor(secretKey.getBytes(java.nio.charset.StandardCharsets.UTF_8));
  }

  public String generateToken(
      String email) { // will return a JWT token as a string, and takes the user's email as input
  ...
  private boolean isTokenExpired(String token) {
    return extractAllClaims(token)
        .getExpiration()
        .before(new Date()); // it compares expiry date with current date(before)
  }
